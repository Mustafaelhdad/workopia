<x-layout>
    <section class="flex flex-col gap-4 md:flex-row">
        ...
        <div class="w-full rounded-lg bg-white p-8 shadow-md">
            ...
            @if ($user->avatar)
                <div class="my-2 flex justify-center">
                    <img class="h-32 w-32 rounded-full object-cover" src="{{ asset('storage/' . $user->avatar) }}"
                        alt="{{ $user->name }}">
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                ...
                <x-inputs.text id='name' name="name" value="{{ $user->name }}" placeholder="Enter Name"
                    label="Name" />
        ...
        <div class="w-full rounded-lg bg-white p-8 shadow-md">
            ...
            @empty
                <p class="text-gray-700">You do not have job listings</p>
            @endforelse
        </div>
    </section>
